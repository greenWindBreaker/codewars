//standup034
//No recursion  or memoization please, just give me a raw for loop for today!
// The Fibonacci numbers, commonly denoted F(n) form a sequence, called the Fibonacci sequence, such that each number is the sum of the two preceding ones, starting from 0 and 1. That is,

// F(0) = 0, F(1) = 1
// F(n) = F(n - 1) + F(n - 2), for n > 1.
// Given n, calculate F(n).

// Example 1:

// Input: n = 2
// Output: 1
// Explanation: F(2) = F(1) + F(0) = 1 + 0 = 1.
// Example 2:

// Input: n = 3
// Output: 2
// Explanation: F(3) = F(2) + F(1) = 1 + 1 = 2.
// Example 3:

// Input: n = 4
// Output: 3
// Explanation: F(4) = F(3) + F(2) = 2 + 1 = 3.
//hint: Create an array [0,1] and then a loop that starts at i =2. Loop all the way to n (the num passed in) add the previous two numbers to get the next number to put in the array. At the end just return the last number
//the whole point of fib is to generate the next num in the sequence - leon
function fib(n){
    let arr = [0,1]//
    for(let i =2; i <= n; i++){//plug the 2 into the arr above
        arr[i] = arr[i-1]+ arr[i-2]//builds slots in our toaster (arr)
    }
    return arr[n]
}
console.log(fib(2))//1
console.log(fib(4))//3